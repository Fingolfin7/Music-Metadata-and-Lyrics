...
def search_song_lyrics(song_name="", song_artist=""):
    def online_search(): 
        ...
        for hit in data['response']['hits']:
            ...
            if result_title.find(song_artist.lower()) != -1 or result_title.find(song_name.lower()) != -1: # if we can find the artist or song name, then we have found our song
                found_song = hit
    ...
    if check_internet(): # if there is an internet connection, run an online search
        count = 0
        print(format_text("Searching [italic][bright yellow]Genius.com[reset] for lyrics"))

        while count < 3: # Try online search 3 times before doing an offline seach
            value = online_search()
            if value is None:
